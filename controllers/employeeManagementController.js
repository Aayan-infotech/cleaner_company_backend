...
exports.addEmployee = [
  ...
  async (req, res, next) => {
    try {
      ...
      return next(
        createSuccess(200, "Employee added successfully.", populatedEmployee)
      );
    } catch (error) {
...
exports.getAllEmployees = async (req, res, next) => {
  try {
    ...
    // Map employee photos to absolute URLs if needed
    const employeesWithPhotoURL = employees.map((emp) => {
      let employeePhotoURL = emp.employee_photo;
      if (employeePhotoURL && !employeePhotoURL.startsWith("http")) {
        employeePhotoURL = `${req.protocol}://${req.get(
          "host"
        )}/uploads/${employeePhotoURL}`;
      }
    ...
    try {
      ...
      const updatedEmployee = await Employee.findByIdAndUpdate(
        id,
        {
          ...
          employee_vanAssigned:
            employee_vanAssigned || employee.employee_vanAssigned,
...
exports.updateEmployeeStatus = async (req, res, next) => {
  try {
    ...
    const newStatus =
      employee.employee_employeeStatus === "Active" ? "Block" : "Active";
...
exports.updatePassword = [
  async (req, res, next) => {
    try {
      ...
      return next(
        createSuccess(200, "Password updated successfully.", updatedEmployee)
      );
    } catch (error) {
      return next(createError(500, error.message || "Internal Server Error!"));
    }
